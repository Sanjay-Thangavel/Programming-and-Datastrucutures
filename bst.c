...
struct bst * widraw(struct bst * root,int acno,int x){
...
case 3:
    ...
    if(temp!=NULL) printf("ACCOUNT NO:%d \n BALNCE :%f\n Name:%s \n Date:%s\n ACCOUNT TYPE :%s \n\n",temp->acno,temp->balance,temp->name,temp->address,temp->date,temp->actype);
...
case 6:
    ...
    scanf("%d",&acno);
    printf("entre the DEPOSIT AMOUNT :\n");
    scanf("%d",&x);
...
case 7 :
   printf("WITHDRAW .....\n Entre the account NO :\n");
    scanf("%d",&acno);
    printf("entre the WITHDRAW AMOUNT :\n");
    scanf("%d",&x);
    root=widraw(root,acno,x);
    break;
